...
const PropertiesPage = () => {
  ...
  const handleToggleActive = async (p) => {
    ...
    try {
      await api.patch(`/properties/${p._id}`, { isActive: !p.isActive });
          ...
          {properties.map(p => (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl ${!p.isActive ? 'bg-slate-500/10' : 'bg-brand-500/10'}`}>
                  {p.isActive ? '🏘️' : '🔒'}
                </div>
                {(isOwner || isSuperAdmin) && (
                  <div className="flex gap-1">
                    ...
                    <button 
                      ...
                      className={`p-1 transition-colors ${p.isActive ? 'text-slate-500 hover:text-danger' : 'text-success hover:scale-110'}`}
              ...
              <h3 className="text-lg font-bold text-white mb-1">
                {p.name}
                {!p.isActive && <span className="ml-2 text-[10px] bg-danger/20 text-danger px-1.5 py-0.5 rounded uppercase">Inactive</span>}
              </h3>
              ...
              <div className="pt-4 border-t border-surface-border flex items-center justify-between">
                ...
                {(isOwner || isSuperAdmin) && p.isActive && (
                  <button 
